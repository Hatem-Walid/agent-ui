...
const ChromaGrid = ({ items, className = '', radius = 300, damping = 0.45, fadeOut = 0.6, ease = 'power3.out' }) => {
  ...
  return (
    ...
    >
      {/* طبقة إضافية لتأثير depth */}
      <div 
        className="absolute inset-0 pointer-events-none"
        style={{
          background: 'radial-gradient(circle at 30% 20%, rgba(120, 119, 198, 0.1) 0%, transparent 50%), radial-gradient(circle at 80% 80%, rgba(255, 119, 198, 0.1) 0%, transparent 50%), radial-gradient(circle at 40% 40%, rgba(120, 219, 255, 0.05) 0%, transparent 50%)',
      ...
      <div className={`relative w-full pt-30 pb-22 flex flex-wrap justify-center items-start gap-5 ${className}`}>
        {data.map((c, i) => (
          <article
            ...
            className="group relative flex flex-col w-[300px] rounded-[20px] overflow-hidden border-2 border-transparent transition-all duration-300 cursor-pointer hover:scale-105 hover:shadow-2xl"
            style={{
          ...
          >
            ...
            <footer className="relative z-10 p-3 text-white font-sans grid grid-cols-[1fr_auto] gap-x-3 gap-y-1">
              ...
              {c.location && <span className="text-[0.95rem] opacity-85 text-right">{c.location}</span>}
            </footer>
          </article>
        ))}
      </div>
      ...
      {/* تأثيرات الـ Mask - تطبق على كل المحتوى بما فيه الـ Footer */}
      <div
      ...
      <div
        ...
        style={{
          ...
          background: 'rgba(0,0,0,0.001)',
           maskImage:
            'radial-gradient(circle var(--r) at var(--x) var(--y),rgba(255,255,255,0.5) 0%,rgba(255,255,255,0.5) 15%,rgba(255,255,255,0.45)30%,rgba(255,255,255,0.39)45%,rgba(255,255,255,0.32)60%,rgba(255,255,255,0.25)75%,rgba(255,255,255,0.16)88%,transparent 100%)', 
           WebkitMaskImage:
            'radial-gradient(circle var(--r) at var(--x) var(--y),rgba(255,255,255,0.5) 0%,rgba(255,255,255,0.5) 15%,rgba(255,255,255,0.45)30%,rgba(255,255,255,0.39)45%,rgba(255,255,255,0.32)60%,rgba(255,255,255,0.25)75%,rgba(255,255,255,0.16)88%,transparent 100%)', 
           opacity: 0
        }}
